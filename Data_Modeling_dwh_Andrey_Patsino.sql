...
insert into dimsupplier (supplierid, companyname, contactname, contacttitle,
...
-- 2.5. Load it into the fact table, as shown below:
...
select d.dateid, c.custid, p.productid, e.empid,  
    cat.categoryid, s.shipperid, sup.supplierid, 
    od.qty, od.unitprice, od.discount,    
...
-- 3.2 Display the top (worst) five products by number of transactions, total sales, and tax
select p.productname, c.categoryname,
    count(*) as numberoftransactions,
    sum(fs.quantitysold * fs.unitprice) as totalsales,
    sum(fs.taxamount) as totaltax
from factsales fs join dimproduct p on fs.productid = p.productid
join dimcategory c on p.categoryid = c.categoryid
group by p.productname, c.categoryname
order by numberoftransactions desc, totalsales desc, totaltax desc limit 5;

-- 3.3 Display the top (worst) five customers by number of transactions and purchase amount
select c.customerid, c.contactname, c.region, c.country,
...
group by c.customerid, c.contactname, c.region, c.country
order by numberoftransactions asc, purchaseamount asc
limit 5;
...
-- 3.5
select dp.categoryid, dc.categoryname,
...
group by dp.categoryid, dc.categoryname, 
